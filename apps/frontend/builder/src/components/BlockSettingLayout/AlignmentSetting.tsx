import { Label } from '@lowcode/shadcn/components/ui/label'
import { Switch } from '@lowcode/shadcn/components/ui/switch'
import { Tabs, TabsList, TabsTrigger } from '@lowcode/shadcn/components/ui/tabs'
...
const Indicator: React.FC<{
  align: AlignGridItemType
  direction?: DIRECTION
  stretch?: boolean
}> = ({ align, direction, stretch }) => {
        ...
        return (
          ...
          >
            <div
              ...
              style={{ ...commonStyle, height: stretch ? 0 : '60%' }}
            />
            <div
              className="bg-primary rounded-sm"
              style={{ ...commonStyle, height: stretch ? '100%' : '90%' }}
            />
            <div
              className="bg-primary rounded-sm"
              style={{ ...commonStyle, height: stretch ? 0 : '40%' }}
            />
          </div>
        )
      }
      case 'center': {
        return (
          ...
          >
            <div
              className="bg-primary rounded-sm"
              style={{ ...commonStyle, height: stretch ? 0 : '50%' }}
            />
  ...
  switch (justifyContent) {
    ...
    case 'flex-end': {
      return (
        <div
          className="flex justify-between size-full"
          style={{ flexDirection: 'column', alignItems: justifyContent }}
        >
          <div
            className="bg-primary rounded-sm"
            style={{ ...commonStyle, width: stretch ? 0 : '60%' }}
          />
          <div
            className="bg-primary rounded-sm"
            style={{ ...commonStyle, width: stretch ? '100%' : '90%' }}
          />
          <div
            className="bg-primary rounded-sm"
            style={{ ...commonStyle, width: stretch ? 0 : '40%' }}
          />
        </div>
      )
    }
    case 'center': {
      return (
        ...
        >
          <div
            className="bg-primary rounded-sm"
            style={{ ...commonStyle, width: stretch ? 0 : '50%' }}
          />
          <div className="bg-primary rounded-sm" style={commonStyle} />
          <div
            className="bg-primary rounded-sm"
          ...
          <div key={row[0]}>
            {row.map(col => {
              ...
              return (
                ...
                >
                  {!selected && (
                    <div className="dot absolute size-[3px] rounded-full bg-zinc-400" />
                  )}
                  {selected && (
                    <Indicator
                      align={col}
                      direction={flexDirection}
                      stretch={isDistributedAlignment}
                    />
                  )}
                </div>
              )
